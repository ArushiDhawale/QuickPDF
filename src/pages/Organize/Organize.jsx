      ...
      >
        <div
          className={`relative rounded-xl overflow-hidden border transition-all duration-150 ${
            isDragOver
              ? "border-white ring-2 ring-white scale-[1.04] bg-zinc-800"
          ...
          {!isProcessing && (
            <div className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                className="p-1.5 bg-red-500/90 hover:bg-red-500 text-white rounded-lg shadow-lg transition-colors"
        ...
        {/* Page number */}
        <div className="mt-1.5 text-center text-xs font-mono text-zinc-500 group-hover:text-zinc-300 transition-colors">
          {pageNumber}
        </div>
      </div>
    </motion.div>
...
export function Organize() {
  ...
  const handleDragEnter = useCallback((index) => {
    if (dragItem.current === index) return; // hovering over itself — ignore
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `QuickPDF_Organized_${file.name}`;
      a.click();
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 bg-zinc-900/50 border border-white/5 rounded-xl">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-medium text-zinc-500 hover:text-white transition-colors disabled:opacity-50"
              >
